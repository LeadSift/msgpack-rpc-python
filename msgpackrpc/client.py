...
class Client(session.Session):
    ...
    def __init__(self, address, timeout=10, loop=None, builder=tcp, reconnect_limit=5, pack_params=dict(encoding='utf-8'), unpack_params=None):
        loop = loop or Loop()
        session.Session.__init__(self, address, timeout, loop, builder, reconnect_limit, pack_params, unpack_params)
