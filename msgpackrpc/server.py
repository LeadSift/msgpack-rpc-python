...
class Server(session.Session):
    ...
    def __init__(self, dispatcher, loop=None, builder=tcp, pack_params=dict(encoding='utf-8'), unpack_params=None):
        self._loop = loop or Loop()
        self._builder = builder
        self._packer_params = (pack_params, unpack_params)
        self._listeners = []
        self._dispatcher = dispatcher

    def listen(self, address):
        listener = self._builder.ServerTransport(address, self._packer_params)
